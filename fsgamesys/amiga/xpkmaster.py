...
class XpkMaster:
    ...
    @staticmethod
    def addFiles(installDir: str, *, files: InstallableFiles) -> None:
        for relativePath, sha1 in XpkMaster.libraryFiles.items():
            files[
                path.join(installDir, relativePath.replace("/", os.sep))
