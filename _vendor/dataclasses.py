...
def _create_fn(
    name, args, body, *, globals=None, locals=None, return_type=MISSING
):
...
def _hash_exception(cls, fields):
    # Raise an exception.
    raise TypeError(
        f"Cannot overwrite attribute __hash__ " f"in class {cls.__name__}"
    )
...
def _astuple_inner(obj, tuple_factory):
    if _is_dataclass_instance(obj):
        ...
        return type(obj)(
            (
                _astuple_inner(k, tuple_factory),
