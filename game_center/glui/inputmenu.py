...
class InputMenu(Menu):
    ...
    def render(self):
        ...
        if len(self.controller.ports) == 0:
            ...
            Render.text(text, Font.main_path_font, 200, 730, 400,
                        color=color, halign=0)
            text = "Press enter or primary key to start game"
            Render.text(text, Font.main_path_font, 200, 670, 400,
                        color=color, halign=0)
            return
        ...
        for port, input in enumerate(self.controller.ports):
            ...
            color = (1.0, 0.0, 0.0, 1.0)
            Render.text(text, Font.main_path_font, center_x - 200, 760, 400,
                        color=color, halign=0)
            text = input.description.upper()
            color = (1.0, 0.0, 0.0, 1.0)
            Render.text(text, Font.main_path_font, center_x - 200, 730, 400,
                        color=color, halign=0)
            if input.device_id:
                device = self.device_data[input.device_id]["device"]
                color = (1.0, 0.5, 0.5, 1.0)
                text = device.name.upper()
                Render.text(text, Font.main_path_font, center_x - 200, 680,
            ...
            for j, device in enumerate(self.devices):
                ...
                Render.text(text, Font.main_path_font, center_x - 200,
